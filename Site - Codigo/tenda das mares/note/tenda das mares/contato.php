<?php
session_start();
?>
<!DOCTYPE html>
<html lang="pt-br">
<head>
  <meta charset="UTF-8">
  <title>Contato</title>
  <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-white text-[#4f2905] font-sans">

  <!-- Navbar -->
  <header class="bg-[#f5f1e6] flex justify-between items-center px-6 py-4 shadow">
    <div>
      <a href="index.php"><img src="img/logo.png" alt="Logo" class="h-10"></a>
    </div>
    <nav class="flex gap-6 text-lg font-medium">
      <a href="produtos.php" class="hover:text-[#b85e2b]">Produtos</a>
      <a href="blog.php" class="hover:text-[#b85e2b]">Blog</a>
      <a href="sobre.php" class="hover:text-[#b85e2b]">Sobre nós</a>
      <a href="contato.php" class="hover:text-[#b85e2b]">Contato</a>
      <a href="login.php">
        <svg class="h-6 w-6 text-[#4f2905]" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" d="M5.121 17.804A4.992 4.992 0 0112 20a4.992 4.992 0 016.879-2.196M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
        </svg>
      </a>
    </nav>
  </header>

  <!-- Conteúdo -->
  <main class="max-w-6xl mx-auto mt-16 grid grid-cols-1 md:grid-cols-2 gap-12 px-6">

    <!-- Card de mídias sociais -->
    <div class="bg-[#fde9c7] p-10 rounded-2xl flex flex-col items-center shadow-lg">
      <h2 class="text-2xl font-semibold mb-6">Mídias sociais</h2>
      <div class="flex gap-6">
        <a href="https://instagram.com/https.m4d" target="_blank">
          <img src="img/instagram.png" alt="Instagram" class="h-12 w-12">
        </a>
        <a href="https://instagram.com/guzzz.osz" target="_blank">
          <img src="img/instagram.png" alt="Instagram" class="h-12 w-12">
        </a>
      </div>
    </div>

    <!-- Card de encomendas -->
    <div class="bg-[#fde9c7] p-10 rounded-2xl flex flex-col items-center shadow-lg">
      <h2 class="text-2xl font-semibold mb-6">Encomendas</h2>
      <a href="http://wa.me//+5541999369485" target="_blank">
        <img src="img/instagram.png" alt="Instagram" class="h-12 w-12">
      </a>
    </div>

  </main>

  <!-- Rodapé opcional -->
  <footer class="mt-16 text-center text-sm text-gray-500 py-6">
    © <?= date("Y") ?> Tenda. Todos os direitos reservados.
  </footer>

</body>
</html>
