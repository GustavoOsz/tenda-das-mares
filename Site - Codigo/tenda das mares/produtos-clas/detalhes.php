...
$stmt = $conn->prepare("SELECT id, nome, preco, imagem FROM produto WHERE id = ?");
  ...
  <main class="grid grid-cols-1 lg:grid-cols-3 gap-6 px-6 py-8 max-w-7xl mx-auto">
    ...
    <div class="lg:col-span-2">
      <div class="bg-[#fde9c7] rounded-xl p-4 mb-4">
        <img src="<?= htmlspecialchars($produto['imagem']) ?>" alt="<?= htmlspecialchars($produto['nome']) ?>" class="w-full h-96 object-cover rounded">
      </div>
    ...
    <div class="bg-[#fde9c7] rounded-xl p-6 flex flex-col justify-between shadow-md">
      <div>
        ...
        <p class="text-xl font-bold mb-4">R$ <?= number_format($produto['preco'], 2, ',', '.') ?></p>
    ...
    <input type="hidden" name="imagem" value="<?php echo $produto['imagem']; ?>" class="border border-[#4f2905] rounded px-2 py-1 text-sm w-24">
    
    <label for="quantidade" class="text-sm font-medium">Quantidade:</label>
    <input type="number" name="quantidade" value="1" min="1">
    
    <button type="submit" class="bg-[#4f2905] hover:bg-[#623613] text-white py-2 rounded text-sm">
...
<?php endif; ?>
    ...
    <div class="lg:col-span-3 mt-6">
      <div class="bg-[#fddda5] rounded-xl p-6 shadow">
        ...
        <p class="text-sm">Produto artesanal selecionado com carinho.</p>
      </div>
    </div>
  </main>
</body>
